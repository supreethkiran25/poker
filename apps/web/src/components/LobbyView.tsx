...
import type { RoomPublicState, BotPersonality, BotDifficulty } from '@poker/shared';
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-8 py-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-3">
            {/* Bored / Solo Player Callout when only 1 player */}
            {playerCount === 1 && (
              <div className="p-4 rounded-3xl bg-gradient-to-r from-purple-950/60 via-indigo-950/40 to-zinc-900 border border-purple-500/40 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-fade-in">
                <div>
                  <div className="font-black text-sm text-purple-200 flex items-center gap-2">
                    <span className="text-lg">🤖</span>
                    <span>Waiting for friends or bored?</span>
                  </div>
                  <div className="text-xs text-zinc-400 mt-0.5">
                    Add AI bots with distinct playstyles and start Texas Hold'em right away!
                  </div>
                </div>
                <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                  <button
                    onClick={() => onAddBot && onAddBot()}
                    className="flex-1 sm:flex-none px-3.5 py-2 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 active:scale-95 text-zinc-950 font-black text-xs rounded-xl shadow transition flex items-center justify-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none px-3.5 py-2 bg-purple-900/60 hover:bg-purple-800 text-purple-200 font-bold text-xs rounded-xl border border-purple-500/40 transition active:scale-95 flex items-center justify-center gap-1.5"
                  >
                  ...
                  >
                    ...
                    <div>
                      {p.isHost ? (
                        ...
                        <div className="flex items-center gap-2">
                          <span className="px-2.5 py-1 bg-purple-950/80 border border-purple-500/40 text-purple-300 rounded-lg text-xs font-mono font-bold flex items-center gap-1.5">
                            <Bot className="w-3.5 h-3.5 text-purple-400" />
                            <span>BOT • {p.personality?.toUpperCase() || 'READY'}</span>
                            {p.difficulty && (
                              <span
                          ...
                          {isHost && onRemoveBot && (
                            roomState.players.length <= 2 ? (
                              <span
                                className="p-1 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-600 cursor-not-allowed text-[10px]"
                                title="Cannot remove: Minimum 2 players needed to start game"
                              >
                              ...
                              <button
                                ...
                                className="p-1 rounded-lg bg-red-950/40 text-red-400 hover:bg-red-900/60 border border-red-500/30 transition text-[10px]"
                                title="Remove Bot"
                              >
          ...
          <div className="md:col-span-1">
            <div className="sticky top-24 p-5 rounded-3xl bg-zinc-900/60 border border-zinc-800 flex flex-col gap-4">
              ...
              <button
                onClick={() => setShowBotModal(true)}
                className="w-full py-3 bg-gradient-to-r from-purple-900/60 via-indigo-900/50 to-purple-900/60 hover:from-purple-800/70 hover:to-indigo-800/70 text-purple-200 font-bold text-xs uppercase tracking-wider rounded-2xl border border-purple-500/40 transition active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-purple-950/40"
              >
